...
QUERY_OUTPUT_DIMENSION_MAP = {
    1: { 1: (4, 10), 2: (460, 8), 3: (11620, 4), 4: (5, 2), 5: (5, 2), 6: (1, 1), 7: (4, 4), 8: (2, 2),
...
def run_queries(parameters):
    ...
    try:  # giant try except: great style
        ...
        end_time = time.time()
        result['runtimes']['run_queries'] = end_time-start_time
        print(f"          {end_time-start_time} seconds")
        result['runtimes']['total'] = sum(result['runtimes'].values())
...
def log_results(result_dict):
    ...
    {   'params': [ list of param dictionaries as described in tpch_param.py ], 
        'runtimes': {'build_config': float time(s) representing time to make spark session and set params
                     ...
                     'run_queries': float time(s) it takes to run and verify all 22 queries
                     'total': float time(s) combining all the above}
